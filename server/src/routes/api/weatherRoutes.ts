...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const cityObj = new (HistoryService as any).City(HistoryService.generateId(), city);
...
router.get('/history', async (_req: Request, res: Response) => {
  try {
    ...
    console.error('Error retrieving search history:', (error as Error).message);
    return res.status(500).json({ message: 'Internal Server Error' });
  }
});
...
router.delete('/history/:id', async (req: Request, res: Response) => {
  try {
    ...
    console.error('Error deleting city from history:', (error as Error).message);
    return res.status(500).json({ message: 'Internal Server Error' });
  }
});
